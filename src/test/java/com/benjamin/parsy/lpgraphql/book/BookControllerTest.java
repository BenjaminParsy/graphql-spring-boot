...
import com.benjamin.parsy.lpgraphql.DataHelper;
...
class BookControllerTest {
    ...
    void createBook_AuthorDoesntExist_ThrowException() {
        ...
        Mockito.when(messageService.getErrorMessage(Mockito.any(), Mockito.any()))
                .thenReturn(new ErrorMessage("[BR1]", String.format("Author with ID %s does not exist", author1.getId())));
        ...
        graphQlTester.documentName("book-test/create-book")
                ...
                .expect(e -> Objects.requireNonNull(e.getMessage())
                        .equals(String.format("[BR1] Author with ID %s does not exist", author1.getId())));
